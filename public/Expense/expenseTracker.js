...
window.addEventListener('DOMContentLoaded', () => {
    ...
    axios.get(`http://https://expense-tracker-t2ho.onrender.com/expense?size=${rows}`, { headers: {'Authorization': token }})
    .then((response) => {
...
function pagination(resData) {
    ...
    if(resData.hasNextPage){
        ...
        nxtBtn.onclick = () => {
            axios.get(`http://https://expense-tracker-t2ho.onrender.com/expense?page=${nxtBtn.textContent}&size=${rows}`, { headers: {'Authorization': token }}).then((response) => {
    ...
    if(resData.hasPreviousPage){
        ...
        prvBtn.onclick = () => {
            axios.get(`http://https://expense-tracker-t2ho.onrender.com/expense?page=${prvBtn.textContent}&size=${rows}`, { headers: {'Authorization': token }}).then((response) => {
                expenseList.innerHTML = "";
                for(expenseObj of response.data.expenses){
                    showExpensesOnScreen(expenseObj);
                }
...
function onSubmit(e){
    ...
    if(expenseDesc.value === '' || expenseAmount.value === ''){
        ...
        if(document.querySelector('#submitBtn').value === 'Update'){
            ...
            axios
              .put('http://https://expense-tracker-t2ho.onrender.com/expense/'+expenseId, expenseObj, { headers: {'Authorization': token }})
              .then((response) => {
            ...
            axios.post('http://https://expense-tracker-t2ho.onrender.com/expense/addExpense', expenseObj, { headers: {'Authorization': token }})
            .then((response) => {
...
function showExpensesOnScreen(obj){
    ...
    delBtn.onclick = () =>{
        if(confirm('Are you sure ?')){
            axios
              .delete("http://https://expense-tracker-t2ho.onrender.com/expense/" + obj._id, { headers: {'Authorization': localStorage.getItem('token') }})
...
function showLeaderBoard() {
    ...
    inputElement.onclick = async() => {
        const token = localStorage.getItem('token')
        const userLeaderBoardArray = await axios.get('http://https://expense-tracker-t2ho.onrender.com/premium/leaderboard', { headers: {"Authorization" : token} })
...
document.getElementById('rzp-button1').onclick = async function (e) {
    const token = localStorage.getItem('token');
    const response = await axios.get('http://https://expense-tracker-t2ho.onrender.com/purchase/premiummembership', { headers: { 'Authorization': token }});
    ...
    var options = 
    {
        ...
        "handler": async function (response) {
            const result = await axios.post('http://https://expense-tracker-t2ho.onrender.com/purchase/updatetransactionstatus', {
    ...
    rzp1.on('payment.failed', async function (response){
        ...
        await axios.post('http://https://expense-tracker-t2ho.onrender.com/purchase/updatetransactionstatus', {
                order_id: options.order_id,
                payment_id: response.razorpay_payment_id,
            }, { headers: {'Authorization': token } })
...
downloadReport = async function (e) {
    try {
        const response = await axios.get('http://https://expense-tracker-t2ho.onrender.com/user/download', { headers: { 'Authorization': token }});
